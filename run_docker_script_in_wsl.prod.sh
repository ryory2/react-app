...
# ログファイルのパスを設定
LOG_FILE="docker_execution_log_in_linux.prod.txt"
...
docker compose --env-file .env.prod config >> "$LOG_FILE" 2>&1
...
docker compose --env-file .env.prod build --no-cache >> "$LOG_FILE" 2>&1

# 起動
echo "■Running docker compose up..." | tee -a "$LOG_FILE"
docker compose --env-file .env.prod up -d >> "$LOG_FILE" 2>&1
